...
def chunk_ast_nodes_by_path(nodes: List[Tuple[List, Node]]) -> List[Tuple[List, List[Node]]]:
	chunks = []
	curr_path = None
	curr_chunk = None
...
def _have_same_references(a_var: str, b_var: str, a_refs: List[Tuple], b_refs: List[Tuple], matches: List[Tuple]) -> bool:
...
def get_renamed_variables(source_code: str, target_code: str) -> Dict[str, str]:
	...
	target_declarations = chunk_ast_nodes_by_path(filter_ast_node(target_tree, VariableDeclaration))

	source_var_names = [declarator.name for path, declarations in source_declarations for declaration in declarations for declarator in declaration.declarators]
	target_var_names = [declarator.name for path, declarations in target_declarations for declaration in declarations for declarator in declaration.declarators]

	source_references = chunk_ast_nodes_by_path(filter_ast_node(source_tree, MemberReference))
	target_references = chunk_ast_nodes_by_path(filter_ast_node(target_tree, MemberReference))
	...
	for i in range(len(source_declarations)):
		...
		for j in range(i, len(target_declarations)):
			...
			prev_matches_here = [(old, new, attrs) for path, v in matches for old, new, attrs in v if path in (source_decn_path, target_decn_path)]

			# Now make sure the paths are equal (ignoring the previously found
			# renamed variables and the declarations themselves)
			if not ast_paths_equal(source_decn_path, target_decn_path, matches=prev_matches_here):
				continue

			for k in range(len(source_decn_nodes)):
				source_decn_node = source_decn_nodes[k]
				for l in range(len(source_decn_node.declarators)):
					source_declarator = source_decn_node.declarators[l]
					# 1b. Must have different names
					if source_declarator.name in target_var_names:
						continue

					sr = [(path, ref) for path, refs in source_references for ref in refs if ref.member == source_declarator.name]

					for m in range(k, len(target_decn_nodes)):
						target_decn_node = target_decn_nodes[m]
						for n in range(len(target_decn_node.declarators)):
							target_declarator = target_decn_node.declarators[n]
							...
							tr = [(path, ref) for path, refs in target_references for ref in refs if ref.member == target_declarator.name]
