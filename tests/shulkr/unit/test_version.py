...
def test_create_version_calls_generate_sources_with_mappings_from_config_and_correct_version(
    mocker, config: Config, empty_repo
):
    ...
    shulkr.version.generate_sources.assert_called_once_with(
        version, "foo", empty_repo.path
    )


def test_create_version_with_undo_renamed_vars_on_repo_with_no_commits_does_not_call_undo_renames(
    mocker, config, empty_repo
):
    # Mock
...
def test_create_version_with_undo_renamed_vars_on_repo_with_one_commit_calls_undo_renames(
    mocker, config, nonempty_repo
):
    # Mock
    # mocker.patch('shulkr.open', create=True)
    mocker.patch("shulkr.version.click")
...
def test_create_version_with_yarn_mappings_stages_the_src_directory(
    mocker, config, empty_repo, yarn_mappings
):
    mocker.patch("shulkr.version.click")
    mocker.patch("shulkr.version.generate_sources")
...
def test_create_version_with_mojang_mappings_stages_the_src_directory(
    mocker, config, empty_repo, mojang_mappings
):
    mocker.patch("shulkr.version.click")
    mocker.patch("shulkr.version.generate_sources")
...
def test_create_version_replaces_all_brackets_with_the_version(
    mocker, config, empty_repo
):
    mocker.patch("shulkr.version.click")
    mocker.patch("shulkr.version.generate_sources")
...
def test_create_version_with_existing_commits_and_undo_renamed_vars_adds_note_to_commit_message(
    mocker, config, nonempty_repo
):
    mocker.patch("shulkr.version.click")
    mocker.patch("shulkr.version.generate_sources")
...
def test_get_latest_generated_version_with_repo_with_one_version_returns_version(
    mocker, nonempty_repo
):
