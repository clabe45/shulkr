...
def test_get_latest_generated_version_with_repo_with_two_versions_returns_newer_version(
    repo: Repo,
):
...
def test_get_latest_generated_version_with_repo_with_two_versions_after_checking_out_older_version_returns_newer_version(
    repo: Repo,
):
    # Add two tagged commits (Minecraft versions)
    repo.git.commit(message="1.17", allow_empty=True)
