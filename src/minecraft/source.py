...
def _generate_sources_with_yarn(version: Version, path: str) -> None:
    ...
    shutil.move(
        os.path.join(
            decompiler_repo.path,
            "src" if version >= Version.of("1.20.5") else "namedSrc",
        ),
        src_path,
    )
...
def _generate_sources_with_mojang(version: Version, path: str) -> None:
    ...
    # Decompile client
    p = subprocess.run(
        ["python3", "main.py", "--mcv", str(version), "-s", "client", "-c", "-f", "-q"],
    ...
    shutil.move(
        os.path.join(decompiler_repo.path, "src", str(version), "client"), dest_src_dir
    )
